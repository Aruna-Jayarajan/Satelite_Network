...
def train_until_converged(
    ...
    sat_positions=None,
    bin_size=1.0,
    satellite_masking_prob=0.8,  # <<-- NEW param: how often to randomly mask
    drop_prob=0.15               # <<-- NEW param: how much satellites to drop
):
    """
    Training loop for SatGatewayCellGNN with optional satellite masking.
    """
    ...
    for epoch in range(1, epochs + 1):
        ...
        # Maybe apply satellite masking (randomly per epoch)
        if random.random() < satellite_masking_prob:
            noisy_data, sat_mask = randomly_mask_satellites(data.clone(), drop_prob=drop_prob)
        else:
            noisy_data = data
        ...
        output = model(
            noisy_data,
            visibility_matrices=visibility_matrices,
        ...
        loss_val, _ = total_loss(
            ...
            cell_sat_probs=output['cell_sat_probs'],   # <--- ADD THIS
            sat_gateway_visibility=aux_inputs['sat_gateway'],
            sat_cell_visibility=aux_inputs['sat_cell'],
            **loss_weights
        )
