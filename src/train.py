import torch
import numpy as np
from loss import compute_mse_loss, get_binary_predictions, top_k_accuracy
from dataloader import prepare_input_for_gnn, NUM_GATEWAYS


def train_model_with_mse(gnn_model, stage1_model, train_loader, val_loader, loss_fn, optimizer_gnn, num_epochs=30, rounds=15):
    ...
    for epoch in range(1, num_epochs + 1):
        gnn_model.train()
        total_loss, total_top1, total_top3, total_top5, total_train_samples = 0, 0, 0, 0, 0

        for data in train_loader:
            optimizer_gnn.zero_grad()

            # Stage 1 predictions
            input_features = data.x[:, :57]
            preds_model1 = stage1_model.predict(input_features.numpy(), verbose=0)
            ...
            # Loss calculation
            cce_loss = loss_fn(preds_gnn_round, data.y)

            predicted_binary = get_binary_predictions(preds_gnn_round)
            true_binary = np.zeros((data.y.shape[0], NUM_GATEWAYS))
            true_binary[np.arange(data.y.shape[0]), data.y.cpu().numpy()] = 1

            if predicted_binary.shape != true_binary.shape:
                min_rows = min(predicted_binary.shape[0], true_binary.shape[0])
                predicted_binary = predicted_binary[:min_rows, :]
                true_binary = true_binary[:min_rows, :]

            mse_loss = compute_mse_loss(predicted_binary, true_binary)
            total_loss_value = cce_loss + 0.5 * mse_loss
        ...
        # Validation
        gnn_model.eval()
        val_loss, val_top1, val_top3, val_top5, total_val_samples = 0, 0, 0, 0, 0
        with torch.no_grad():
            for data in val_loader:
                input_features = data.x[:, :57]
                preds_model1 = stage1_model.predict(input_features.cpu().numpy(), verbose=0)
                top3_model1 = np.argsort(preds_model1, axis=1)[:, -3:]
                binary_preds_model1 = np.zeros_like(preds_model1)
                for i, idx in enumerate(top3_model1):
                    binary_preds_model1[i, idx] = 1
